...
export async function DownloadReportRating (startDate: string, endDate: string, token: string) {
  Swal.fire({
    title: 'Aguarde...',
  ...
  await axios.get(`http://100.28.169.213/api/relatorios/PDF/avaliacao-por-prestador?dataInicio=${startDate}&dataFim=${endDate}`, {
    headers: {
    ...
    tempLink.href = url
    tempLink.download = 'avaliacao_por_prestador.pdf'  // Nome do arquivo que será baixado
